    ...
    for (let i = 0; i < len; i++) {
      for (let j = 0; j < len - 1; j++) {
        if (this.squares[j].value > this.squares[j + 1].value) {
          ...
          this.squares[j].move(this.positions[j + 1]);
          this.squares[j].anim();

          this.squares[j + 1].move(this.positions[j]);
          this.squares[j + 1].anim();
...
class Square {
  ...
  draw(x = this.posX, y = this.posY) {
    ...
    ctx.fillText(
      this.value,
      this.value <= 9 ? x + this.width / 3 : x + this.width / 5.5,
      y + (2 * this.height) / 3
    );
  }
  ...
  anim(time) {
    ...
    if (deltaTime >= 1) {
      // this means we ended our animation
